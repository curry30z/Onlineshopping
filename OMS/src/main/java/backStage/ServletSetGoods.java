...
@WebServlet(name = "ServletSetGoods", value = "/ServletSetGoods")
@MultipartConfig( location = "F:/OMS/web/backStage/img", maxFileSize = 1024*1024*10*100)
public class ServletSetGoods extends HttpServlet {
    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //更新的数据
            ...
            if(goodsno==null||goodsno.equals("")){
                ...
                List<goods> list =dao.SelectGoodsById(id);
                goods  g= list.get(0);
                session.setAttribute("o",g);
                response.sendRedirect("backStage/updateGoods.jsp");
                ...
                try {
                    ...
                    out.print("   <script>alert('输入商品类别有误！');window.location.href='backStage/updateGoods.jsp' </script> ");
                    return;
...
//图片存入
                ...
                int rs = -1;
                goods s = new goods(goodsno, gname, price, amount, fileName, seller, name, adddate, discount, describe);
                s.setId(id);
                rs = dao.updateByGoods(s);


                if (rs > 0) {
                    out.print("   <script>alert('修改商品成功！');window.location.href='backStage/Manage.jsp' </script> ");

                } else {
                    out.print("   <script>alert('修改商品失败');window.location.href='backStage/Manage.jsp' </script> ");
